class Combobox {
  constructor(input, list, { tabInsertsSuggestions, defaultFirstOption } = {}) {
    this.input = input
    this.list = list
    this.tabInsertsSuggestions =
      tabInsertsSuggestions !== null && tabInsertsSuggestions !== void 0
        ? tabInsertsSuggestions
        : true
    this.defaultFirstOption =
      defaultFirstOption !== null && defaultFirstOption !== void 0
        ? defaultFirstOption
        : false
  ...
  indicateDefaultOption() {
    ...
    if (this.defaultFirstOption) {
      ...
      ).filter(visible)[0]) === null || _a === void 0
        ? void 0
        : _a.setAttribute("data-combobox-option-default", "true")
    }
  }
  navigate(indexDiff = 1) {
    const focusEl = Array.from(
    ...
    if (
      (focusIndex === els.length - 1 && indexDiff === 1) ||
      (focusIndex === 0 && indexDiff === -1)
    ) {
      this.clearSelection()
      this.input.focus()
      return
...
function keyboardBindings(event, combobox) {
  ...
  switch (event.key) {
    ...
    case "Tab":
      if (
        combobox.tabInsertsSuggestions &&
        commit(combobox.input, combobox.list)
      ) {
        event.preventDefault()
      }
      break
...
function trackComposition(event, combobox) {
  ...
  const list = document.getElementById(
    combobox.input.getAttribute("aria-controls") || "",
  )
  if (!list) return
  combobox.clearSelection()
}
...
function query(
  ...
  key,
  cursor,
  { multiWord, lookBackIndex, lastMatchPosition } = {
    multiWord: false,
    lookBackIndex: 0,
...
class TextExpanderElement extends HTMLElement {
  get keys() {
    ...
    const globalMultiWord =
      multiWord.length === 0 && this.hasAttribute("multiword")
    return keys.map((key) => ({
      key,
